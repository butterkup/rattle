  ...
  Scoped<tree::Stmt> Parser::parse_statement() noexcept {
    for (;;) {
      ...
      case token::Kind::Continue: {
        auto kw = base.eat();
        parse_eos();
        return make<tree::Continue>(kw);
      }
      case token::Kind::Break: {
        auto kw = base.eat();
        parse_eos();
        return make<tree::Break>(kw);
      }
      case token::Kind::Return: {
        auto kw = base.eat();
        auto value = parse_expression();
        parse_eos();
        return make<tree::Return>(kw, std::move(value));
      }
      case token::Kind::Nonlocal: {
        auto kw = base.eat();
        auto names = parse_expression();
        parse_eos();
        return make<tree::Nonlocal>(kw, std::move(names));
      }
      case token::Kind::Global: {
        auto kw = base.eat();
        auto names = parse_expression();
        parse_eos();
        return make<tree::Global>(kw, std::move(names));
      }
